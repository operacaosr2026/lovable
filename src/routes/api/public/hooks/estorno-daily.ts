...
// + congela o realizado das metas da empresa de meses que fecharam.
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const [estorno, stores, goals] = await Promise.all([runEstornoDaily(), runStoreMetricsDaily(), freezeAllCompanyGoals()]);
          return Response.json({ estorno, stores, goals });
        } catch (e: any) {
