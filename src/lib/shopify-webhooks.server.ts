...
// Webhooks da Shopify (pedido criado/alterado) apontando pro nosso endpoint
...
export const SHOPIFY_WEBHOOK_TOPICS = ["orders/create", "orders/updated"] as const;
...
export async function ensureShopifyWebhooks(store: Store): Promise<{ created: string[]; skipped?: boolean }> {
  ...
  for (const topic of SHOPIFY_WEBHOOK_TOPICS) {
    ...
    if (!r.ok) throw new Error(`Shopify criar webhook ${topic} ${r.status}: ${(await r.text()).slice(0, 200)}`);
    created.push(topic);
  }
  return { created };
}
